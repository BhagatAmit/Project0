										  ...
										  int ch2=0;
										   do {
											...
											//	log.info("\n4)View Customer Account Details");
												log.info("\n2)View Transaction of Customers");
												log.info("\n3)Logout");
...
//												String userName = sc.nextLine();
												...
												//	System.out.println(currbalance);
													if(accountpresent!=0)
													{
														...
														if(withdrawlAmount>0 && withdrawlAmount<currbalance)
															valid1=true;
														else
												...
												case 5:
													...
													do {
													 accountno1=Long.parseLong(sc.nextLine());
													long accountpresent=0;
													accountpresent=bankSearchService.getCustomerByAccountno(accountno1);
													if(accountpresent!=0)
														valid11=true;
													else
														log.info("Account not found please re-Enter the account no.");
													}while(!valid11);
													double currbalance=bankSearchService.getCurrbalanceByAccountno(accountno1);
												
													log.info(currbalance);
													}catch (Exception e) {
			...
			case 2: BankCrudService bankCrudService =null;
						...
						str=bankSearchService.getCustomerByUsername(userName);
						if(str==null)
							valid=true;
						else
							log.info("Username already Taken... Please try a different one");
				...
				try {
					...
					log.warn(e.getMessage());
					throw new BankingException("Customer not Registered!!");
				};
					break;
			case 3:
				log.info("Enter your username");
				...
				try {
				 String custUserName=sc.nextLine();
				String str1=bankSearchService1.getCustomerByUsername(custUserName);
